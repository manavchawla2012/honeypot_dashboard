...
(function (window) {
    ...
    */
    ...
    var settings = {
        ...
        // Websocket settings
        //wsHost: "ws://mbsd.ipviking.com:443/",
...
*/
        ...
        pushAttack: function (attack) {
            ...
            if (this.linkSiblings) {
                ...
                this.nodes.forEach(function (n) {
                    if (that._mapKey(n) === key) {//&& n.dst_port === d.dst_port ) {
                        that.links.push({
                            source: n,
                            target: attack,
                            pruneTS: (new Date()).getTime() / 1000 + settings.dataPruneInterval,
                            linkStrength: n.dst_port === attack.dst_port ? 0.5 : 0.25
                        });
                    }
                });
            }
    ...
    var LinksModel = {
        // {ORIGINCOUNTRY: {ORIGINCITY: {TRGTCOUNTRY: {TRGTCOUNTRY: {dst_port: COUNT}}}}}
        ...
        cityToLinks: function (city, strokeOrigin, strokeTarget) {
            ...
            for (var country in city.counts) {
                ...
                for (var cityKey in city.counts[country]) {
                    ...
                    for (var dst_port in counts) {
                        var r = circleScale(counts[type_to_int[dst_port]]),
                            color = colorizer(type_to_int[dst_port]),
                        ...
                        links.push({
                            count: counts[dst_port],
                            source: source,
                            target: target,
                            width: lineScale(counts[type_to_int[dst_port]]),
                            color: color
                        });
                    }
                }
        ...
        dst_portLinks: function (dst_port) {
            var links = []
                ...
                for (var sourceCity in this._links[sourceCountry]) {
                    ...
                    for (var targetCountry in s.counts) {
                        for (var targetCity in s.counts[targetCountry]) {
                            ...
                            for (var targetdst_port in s.counts[targetCountry][targetCity]) {
                                var c = s.counts[targetCountry][targetCity][targetdst_port];
                                if (dst_port && dst_port === targetdst_port) {
                                    links.push({
                                        count: c,
                                        source: s,
                                        target: t,
    ...
    var countryModel = {
        ...
        getByIso2: function (iso2) {
            return this._iso2[iso2 !== undefined ? iso2.toUpperCase() : "IN"];
        },
            ...
            nodes: {
                ...
                draw: function (context) {
                    ...
                    nodeModel.get(nodeModel.ATTACKS)
                        .forEach(function (n) {
                            var c = d3.rgb(colorizer(type_to_int[n.dst_port])),
                                r = that.getRadius(n);
                ...
                draw: function (context) {
                    ...
                    for (var i = 0; i < nodeModel.nodes.length; i++) {
                        ...
                        if (n.type === nodeModel.ATTACKS && n.age < this.duration) {
                            ...
                            context.strokeStyle = colorizer(type_to_int[n.dst_port]);
                            context.lineWidth = 3;
            ...
            if (nearest && nearest.distance < 20) {
                ...
                if (key !== previous) {
                    ...
                    displayLabel.set(msg +
                        ((nearest.city == null || nearest.city === "" || nearest.city.length <= 0) ? "<unknown>" : (nearest.city + ", "))
                        + nearest.country) //countryModel.getByIso2(nearest.country).country);
                }
            } else {
                previous = key;
        ...
        stats: [
            ...
            new Stats(
                {state: d3.map(),
                 ...
                 insert: function(incoming) {
                     this.updated = incoming.dst_port;
                     if (this.state.has(incoming.dst_port)) {
                         this.state.set(incoming.dst_port,
                                        this.state.get(incoming.dst_port) + 1);
                     } else {
                         this.state.set(incoming.dst_port, 1);
                     }
                 },
                 ...
                 redraw: function() {
                     ...
                     rows.enter()
                         ...
                         .on("mouseenter", function(d) {
                             painter.drawings.links.data =
                                 linkModels.origins.dst_portLinks(d.key).map(function(c) {
                                     var r = circleScale(c.count),
                                         color = colorizer(c.dst_port);
                                     return {
                                         source: {
                                             x: c.source.pt[0],
                                             y: c.source.pt[1],
            ...
            new Stats({
                ...
                redraw: function () {
                    var that = this;
                    var rows = d3.select("#events-data").selectAll("tr.table_row")
                    ...
                    rows.enter().append("tr")
                        .style("color", function (d) {
                            return colorizer(type_to_int[d.dst_port]);
                        })
                    ...
                    var cols = rows.selectAll("td")
                        .data(function (d) {
                            return [
                                d.datetime,
                                spanWrap(
                                    ((d.city === null || d.city === undefined ) ? "" : (d.city + ", ")) +
                                    typeof (countryModel.getByIso2(d.countrycode)) === 'undefined' ? d.country : countryModel.getByIso2(d.countrycode).country,
                                    ["location", "overflow"]),
                                d.source_ip,
                                d.dst_port,
                                spanWrap(
                                    ((d.city2 === null || d.city2 === undefined ) ? "" : (d.city2 + ", ")) +
                                    typeof (countryModel.getByIso2(d.countrycode2)) === 'undefined' ? d.country2 : countryModel.getByIso2(d.countrycode2).country,
    ...
    function start(loc, psk) {
        ...
        webSocket.trigger = function (datum) {
            var datum = datum;
            ...
            datum.id = getID();
            datum.datetime = (new Date()).toLocaleString()
                .replace("T", "&ensp;")
                .slice(0, -2);
 ...
 */
    ...
    queue()
        ...
        .await(function (error, world, countryCodes) {
            ...
            var data_attr = $('#content').data()
            var interval_value = data_attr["intervalValue"]
            var map_color = data_attr["mapColor"]
            var refresh_time =
                // Enter the countries
                svg.append("g")
                    .attr("class", "world")
                    ...
                    //.attr("fill", 'rgba(100, 186, 229, 0.6)')
                    .attr("fill", map_color)
                    .attr("d", path);
            ...
            function doScaledTimeout(datum, i) {
                setTimeout(function () {
                    webSocket.trigger(datum);
                }, i * interval_value);
            }
                ...
                if ($('#content').length) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            var data = data
                            var i = 0;
                            var date_time, dst_port
                            data.forEach(function (datum, i) {
                                dst_port = datum["dst_port"]
                                type_to_int[dst_port] = dst_port
                                //datum["timestamp"] = date_time.toLocaleString('en-US', { timeZone: 'Asia/Kolkata' })
                                doScaledTimeout(datum, i);
                            });
                        },
                        error: function () {
                            console.log("error");
                        ...
                        complete: function () {
                            get_attack_data();
                            setTimeout(getData, (number_of_records * interval_value))
                        }
                    });
                }
            })();
            ...
            function get_attack_data() {
                $.ajax({
                    ...
                    headers: { "X-CSRFToken": csrftoken },
                    success: function (data) {
                        var left_element = $("#left-data tbody");
                        left_element.find("tr:gt(0)").remove();
                        data.attacks.forEach(function (attack, i) {
                            $('#top-10-attack')
                                .append($("<option></option>")
                                    .attr("value", attack)
                                    .text(attack));
                            left_element.append("<tr class='node'><td></td><td>" + (i + 1) + "</td><td class='attack' title='" + attack + "'>" + attack + "</td></tr>");
                        })
                    },
                    complete: function () {
                        get_country_data();
            ...
            function get_country_data() {
                $.ajax({
                    url: "/get_attacking_countries",
                    dataType: 'json',
                    method: "post",
                    headers: { "X-CSRFToken": csrftoken },
                    success: function (data) {
                        var right_element = $("#right-data tbody");
                        right_element.find("tr:gt(0)").remove();
                        data.countries.forEach(function (country, i) {
                            var countryCode = countryModel.getByCountry(country)
                            countryCode = flagTag(countryCode != null ? countryCode.iso2 : "")
                            $('#top-10-country')
                                .append($("<option></option>")
                                    .attr("value", countryCode)
                                    .text(country));
                            right_element.append("<tr class='node'><td></td><td>" + (i + 1) + "</td><td>" + countryCode + "</td><td class='country'>" + country + "</td></tr>");
                        })
                    }
                })
            }
