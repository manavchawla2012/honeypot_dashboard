...
require.config({
    paths: {
        "test": "/static/javascript/stix/stix2viz/d3/d3"
    }
});
...
require(["domReady!", "stix2viz/stix2viz/stix2viz"], function (document, stix2viz) {
    ...
    function vizStixWrapper(content, customConfig) {
        cfg = {
            iconDir: "/static/images/stix_icons"
        }
     ...
     * ******************************************************/
    function handleTextarea(stix_data) {
        customConfig = "";
        content = stix_data;
...
is not serving JSON, or is not running a webserver.\n\nA GitHub Gist can be created to host RAW JSON data to prevent this.");
    ...
    function refresh_node() {
        $('#canvas').html("");
        $('.sidebar-stix-content').html("");
    }

    function stix_operations() {
        url = new URL(window.location.href);
        parameters = new URLSearchParams(url.search.slice(1));
        operation_id = parameters.get("operation");
        id = parameters.get("id");
        type = parameters.get("stix-type");
        is_drill = parameters.get("drill");
        $.ajax({
            url: "/stix/get_stix_file",
            type: "post",
            dataTyle: "json",
            data: $.param({stix_type: type, data_id: id, drill: is_drill}),
            success: function (data) {
                if (data.success) {
                    text = data.data.replace(/\\/g, "");
                    var input = eval('(' + text.substring(1, text.length - 1) + ')');
                    switch (operation_id) {
                        case "1":
                            refresh_node();
                            handleTextarea(input);
                            break;
                        case "2":
                            var data = "text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(input));
                            json_download = document.getElementById('json-download');
                            json_download.setAttribute("href", data);
                            json_download.setAttribute("download", "data.json");
                            json_download.click();
                            //window.close();
                            break;
                        case "3":
                            $('.json-data').toggleClass("hidden");

                            $('.json-data').jsonViewer(input, "");
                            break;
                        default:
                            window.close();
                            break;
    ...
    function test() {
        let data = $("#content-data").data("stix")

        let json_data = getUrlParameter("json")
        let download = getUrlParameter("download")
        if (Object.keys(data).length > 0 && data.constructor === Object) {
            if (download === "True" || download === "true" || download === 1) {
                data = "data:text/json;charset=utf-8," + JSON.stringify(data);
                let json_download = $('#json-download');
                json_download.attr("href", data);
                json_download.attr("download", "data.json");
                json_download[0].click()


                //window.close()
            } else {
                if (json_data === "True" || json_data === "true" || json_data === 1) {
                    $('.json-data').toggleClass("hidden");

                    $('.json-data').jsonViewer(data, "");
                } else {
                    refresh_node()
                    handleTextarea(data);
                }
